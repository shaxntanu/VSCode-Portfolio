    ...
    const shuffleText = (originalText: string, currentRevealed: Set<number>): string => {
      if (useOriginalCharsOnly) {
        ...
        const nonSpaceChars = positions
          .filter((p: { char: string; isSpace: boolean; index: number; isRevealed: boolean }) => !p.isSpace && !p.isRevealed)
          .map((p: { char: string; isSpace: boolean; index: number; isRevealed: boolean }) => p.char);
        ...
        let charIndex = 0;
        return positions
          .map((p: { char: string; isSpace: boolean; index: number; isRevealed: boolean }) => {
            if (p.isSpace) return ' ';
